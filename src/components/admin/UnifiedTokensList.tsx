...
const UnifiedTokensList = () => {
  ...
  useEffect(() => {
    load();
    const channels = (["storno_tokens", "limit_tokens", "pin_tokens", "auth_tokens", "panel_task_meta"] as const).map(t =>
  ...
  const handleDeleteAll = async () => {
    ...
    const tables = ["storno_tokens", "limit_tokens", "pin_tokens", "auth_tokens"] as const;
          ...
          {rows.length === 0 ? (
            ...
            <div className="space-y-2">
              {rows.map(r => {
                ...
                const KindIcon = isAuth ? ShieldCheck : isPin ? KeyRound : isLimit ? Sliders : Ban;
                const kindLabel = isAuth ? "Auth" : isPin ? "PIN" : isLimit ? "Limit" : "Storno";
                return (
